...
class UserManager(models.Manager):
    def editUser(self, postData):
        ...
        email_val = self.filter(~Q(email = postData['email']))
        if postData['email'] in email_val:
            errors['reg'] = "Email is already in use"
        return errors

    def editPassword(self, postData):
        errors = {}
        if len(postData['new_password']) < 8:
            errors['reg'] = "Password cannot be less than 8 characters"
        if len(postData['new_password']) > 100:
        ...
        if bcrypt.checkpw(postData['old_password'].encode(), user.password.encode()):
            return errors
        else:
